...
<script>
  ...
  function createPDF() {
    ...
    // Add title
    doc.setFontSize(16);
    doc.text("LAPORAN DATA PENGEMBALIAN", 105, 15, null, null, "center");
    doc.setFontSize(12);
    ...
    const rows = [
      <?php foreach ($loans as $index => $loan):
        ...
        $isFined = $loan['fine_id'] != null;
        $isFinePaid = $isFined ? (($loan['amount_paid'] ?? 0) >= $loan['fine_amount']) : true;
        $status = $isFinePaid ? ($isFined ? 'Lunas' : 'Selesai') : 'Menunggak';
      ?>[
          <?= $index + 1 ?>,
          "<?= addslashes($loan['first_name'] . ' ' . $loan['last_name']) ?>",
          "<?= addslashes($loan['title'] . ' (' . $loan['year'] . ')') ?>",
          "<?= addslashes($loan['quantity']) ?>",
          "<?= addslashes($loanCreateDate->toLocalizedString('dd/MM/y HH:mm:ss')) ?>",
          "<?= addslashes($loanDueDate->toLocalizedString('dd/MM/y')) ?>",
          "<?= addslashes($loanReturnDate->toLocalizedString('dd/MM/y HH:mm:ss')) ?>",
          "<?= addslashes($status) ?>"
        ],
      <?php endforeach; ?>
    ];
    ...
    doc.autoTable({
      startY: 30,
      head: [
        ['No', 'Nama Peminjam', 'Judul Buku', 'Jumlah', 'Tgl Pinjam', 'Tenggat', 'Tgl Pengembalian', 'Status']
      ],
      body: rows,
      styles: {
        fontSize: 8,
    ...
    // Save the PDF
    doc.save("laporan-pengembalian-<?= date('Y-m-d') ?>.pdf");
  }
</script>
